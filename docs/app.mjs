import {tr as uiTr} from './review-ui.mjs';
...
function renderLanguage(){
 document.documentElement.lang=lang==='zh'?'zh-CN':'en';document.title='Delivery Note · '+(lang==='zh'?'数据与运行检查':'Data & runtime checks');
...
function renderFiles(){
 $('files').replaceChildren();for(const [i,f]of pkg.files.entries()){
  ...
  info.append(node('span',f.name,'file-name'),node('span',`${fileBytes(f).length.toLocaleString()} ${lang==='zh'?'字节':'bytes'} · UTF-8`,'file-size'));
  const role=node('select');role.setAttribute('aria-label',f.name+(lang==='zh'?' 的材料角色':' role'));for(const value of ['reference','deliverable']){const o=node('option',t(value));o.value=value;role.append(o);}role.value=f.role;
...
function renderReport(report){
 ...
 for(const r of ordered){
  ...
  for(const id of r.evidence_ids){const e=report.evidence.find(x=>x.id===id);item.append(node('p',observation(e),'observation'),node('p',`${id} · ${methodName(e.method)} · ${e.sources.map(x=>x.file).join(' ↔ ')||'—'}`,'evidence-source'));const d=node('details');d.append(node('summary',t('rawEvidence')),node('pre',JSON.stringify(e,null,2)));item.append(d);}
  $('results').append(item);
 }
 $('inventory').replaceChildren();for(const f of report.inventory)$('inventory').append(node('div',`${f.name} · ${t(f.role)} · ${f.bytes} ${lang==='zh'?'字节':'bytes'}\nSHA-256 ${f.sha256}`,'inventory-entry'));
...
function readableMarkdown(r){
 const a=['# '+(lang==='zh'?'数据与运行检查报告':'Data & runtime review'),' ',t(r.status),'',r.objective,''];
 for(const x of r.results){a.push('## '+x.id+' · '+t(x.status)+' · '+bilingual(x.title),'');for(const id of x.evidence_ids){const e=r.evidence.find(y=>y.id===id);a.push(observation(e),'',t('rawEvidence')+':','',...e.observation.split('\n').map(z=>'> '+z),'');}}
 a.push('## '+t('limitsTitle'),'',t('limitsBody'),'',t('snapshotLabel'),r.snapshot,r.created_at);return a.join('\n');
}
function download(kind){if(!lastReport)return;const text=kind==='json'?JSON.stringify(lastReport,null,2):readableMarkdown(lastReport);const url=URL.createObjectURL(new Blob([text],{type:kind==='json'?'application/json;charset=utf-8':'text/markdown;charset=utf-8'}));const a=node('a');a.href=url;a.download='delivery-review-'+lastReport.snapshot.slice(0,10)+'.'+kind;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);}
