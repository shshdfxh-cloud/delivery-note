...
2. In a new TAB of the existing signed-in SERV Playground window, submit the job's system + prompt. Preserve existing drafts and tabs. No new browser window, profile, cookie extraction, key extraction, payment or account-setting change is needed. The comparison Playground may run both raw and SERV models; one submission is not necessarily one billed model request. Do not resubmit automatically after timeouts.
